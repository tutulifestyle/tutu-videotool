...
function bindEvents() {
    ...
    document.getElementById('editDefaultPromptsBtn').addEventListener('click', openDefaultPromptModal);

    // 默认Prompt 配置
    document.getElementById('defaultPromptModalClose').addEventListener('click', closeDefaultPromptModal);
    document.getElementById('saveDefaultPromptsBtn').addEventListener('click', saveDefaultPrompts);
...
function openDefaultPromptModal() {
    const modal = document.getElementById('defaultPromptModal');
    modal.classList.add('show');
    loadDefaultPrompts();
}

function closeDefaultPromptModal() {
    const modal = document.getElementById('defaultPromptModal');
    modal.classList.remove('show');
}

function loadDefaultPrompts() {
    // 加载当前的默认Prompt（可能是自定义的，也可能是初始的）
    const currentDefaults = getDefaultPrompts();

    document.getElementById('defaultTitlePrompt').value = currentDefaults.title;
    document.getElementById('defaultSummaryPrompt').value = currentDefaults.summary;
    document.getElementById('defaultTagsPrompt').value = currentDefaults.tags;
}

function saveDefaultPrompts() {
    const newDefaultPrompts = {
        title: document.getElementById('defaultTitlePrompt').value,
        summary: document.getElementById('defaultSummaryPrompt').value,
        tags: document.getElementById('defaultTagsPrompt').value
    };

    // 保存为新的默认Prompt
    localStorage.setItem('defaultPrompts', JSON.stringify(newDefaultPrompts));

    showMessage('defaultPromptMessage', '默认Prompt已保存！下次还原时将使用此配置', 'success');
